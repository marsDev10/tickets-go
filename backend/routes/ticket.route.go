...
func DeleteTicketHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		utils.JSONResponse(w, http.DefaultMaxHeaderBytes, utils.ErrorResponse("No autorizado", err.Error()))
		return
	}
	...
	if err := controllers.DeleteTicket(claims.OrganizationID, id); err != nil {
		utils.JSONResponse(w, http.StatusBadRequest, utils.ErrorResponse("Error al eliminar ticket", err.Error()))
		return
	}
